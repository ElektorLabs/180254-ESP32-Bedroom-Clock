...
void eepwrite_ledsettings(displaysettings_t data){
  eepwrite_struct( ( (void*)(&data) ), sizeof(displaysettings_t) , LEDSETTINGS_START );  
}
...
displaysettings_t eepread_ledsettings( void ){
  displaysettings_t retval;
  if(false == eepread_struct( (void*)(&retval), sizeof(displaysettings_t) , LEDSETTINGS_START ) ){ 
    Serial.println("MQTT CONF");
    bzero((void*)&retval,sizeof( displaysettings_t ));
    retval.ledlevel=UINT16_MAX;
    eepwrite_ledsettings(retval);
  }
  return retval;
}
